 ...
 * Time: 17:41
 */
class AContentChild extends CModel
{
    ...
    {
        return array(
            array('id,is_irating,is_associated,is_awards,is_professional,is_education,is_startyear,price,email,status,author,irating,rating,is_desc,is_img,is_mtitle,is_mdesc,is_mkeys,is_clinic,reviews,fio,sames,speciality,startyear','safe'),
            array('id,is_irating,is_associated,is_awards,is_professional,is_education,is_startyear,price,email,status,author,irating,rating,is_desc,is_img,is_mtitle,is_mdesc,is_mkeys,is_clinic,reviews,fio,sames,speciality,startyear','safe','on'=>'search'),
        );
    }
    ...
    {
        return array('id','fio','translit','price','speciality','status','rating','is_irating','is_desc','is_img','reviews','is_startyear','is_education','is_associated','is_awards','is_professional','email','author','irating','is_mtitle','is_mdesc','is_mkeys','is_clinic','startyear');
    }

    public function attributeNamesSmall()
    {
        return array('id','fio','translit','price','speciality','status','rating','is_irating','is_desc','is_img','reviews','is_startyear','is_education');
    }
    ...
    private function getContent()
    {
        ...
        $command=Yii::app()->db->createCommand('SELECT
            ...
            rs.rate AS irating,
            (SELECT COUNT(id) FROM child_reviews WHERE (doctor_id=d.id)) AS reviews
        FROM child_doctor as d
            LEFT JOIN child_clinic_doctor dc ON d.id = dc.doctor_id AND dc.speciality IS NOT NULL AND dc.position IS NOT NULL
            LEFT JOIN child_clinic c ON dc.clinic_id = c.id
            LEFT JOIN child_subway_clinic ds ON c.id = ds.clinic_id
            LEFT JOIN child_doctor_rating rs ON d.id = rs.nid
