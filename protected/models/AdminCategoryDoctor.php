...
class AdminCategoryDoctor extends CActiveRecord
{
    ...
    public function getParent_name()
    {
        if(!empty($this->parent_id)){
            $model=AdminCategoryDoctor::model()->findByPk($this->parent_id);
            if(!empty($model->name)){
    ...
    public function rules()
    {
        return array(
            ...
            array('category_id','exist','attributeName'=>'id','className'=>'AdminCategory','message'=>'Услуги не обнаружено'),
            array('doctor_id','exist','attributeName'=>'id','className'=>'AdminDoctor','message'=>'Клиника не обнаружена'),
            array('parent_id','exist','attributeName'=>'id','className'=>'AdminCategoryDoctor','message'=>'Родительская услуга не обнаружена'),
    ...
    {
        return array(
            'catd_category'=>array(self::BELONGS_TO,'AdminCategory','category_id'),
            'catd_doctor'=>array(self::BELONGS_TO,'AdminDoctor','doctor_id'),
            'catd_service'=>array(self::BELONGS_TO,'AdminCategoryDoctor','parent_id'),
        );
    }
